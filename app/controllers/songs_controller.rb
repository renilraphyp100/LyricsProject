class SongsController < ApplicationController
  ...
  def find_songs
    songs = Song.includes(:album, :artist).paginate(:page => params[:page], :per_page => 2)
    songs = songs.order(updated_at: :desc) if !params[:sort_by].present?
    songs = songs.order(:title) if (params[:sort_by].present? and params[:sort_by] == 'title')
    songs
  end
